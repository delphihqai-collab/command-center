...
**Current implementation spec:** `docs/implementation-spec-v5.md`  
**Previous specs:** implementation-spec-v4.md (archived), implementation-spec-v3.md (archived)

### V5 Architecture Additions (above V4)

New routes:
- `alerts/` — Alert rules + alert events, Realtime
- `reports/` — Weekly commercial reports (generated by HERMES)
- `agents/[slug]/` — Individual agent detail + calibration tracker

New API routes:
- `api/search/` — Full-text search across leads/clients/proposals
- `api/hermes/approval-decision/` — HERMES polls for in-app approval decisions

New components:
- `components/command-palette.tsx` — cmd+K global search
- `components/realtime-table.tsx` — Generic Realtime wrapper
- `components/theme-toggle.tsx` — Dark/light mode toggle

New lib:
- `lib/supabase/service.ts` — Service role client (bypasses RLS)
- `lib/lead-scoring.ts` — Lead score calculator

New DB tables (V5 migrations 12–18):
- `alert_rules` — configurable alert conditions
- `alert_events` — fired alert instances
- `calibration_gates` — agent calibration progress tracking
- `weekly_reports` — Friday auto-generated commercial reports
- `client_notes` — HERMES notes on clients
- `notification_preferences` — per-user alert routing preferences
- `pipeline_config` — configurable pipeline settings

### HERMES → Supabase Integration (V5 key feature)

HERMES writes events to Supabase via REST API using `SUPABASE_SECRET_KEY`. Skill: `workspace/skills/supabase-write/SKILL.md`. All heartbeats, lead changes, approvals, and token usage now flow into the DB rather than only to markdown files.
