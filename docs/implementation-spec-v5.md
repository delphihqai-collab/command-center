...
## 1. ~~HERMES → Supabase Integration~~ — REMOVED

This section was incorrectly specced. HERMES is the Commercial Director — not a database engineer. Direct DB writes from HERMES using a service role key is an architectural decision that belongs to ATLAS and the engineering team, not to the commercial department.

The correct future integration path: ATLAS designs and builds the agent reporting API. HERMES calls a controlled endpoint. That work is scoped separately and is not part of this spec.
